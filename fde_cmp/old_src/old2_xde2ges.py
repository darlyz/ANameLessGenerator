...
def xde2ges(gesname,keywd_tag,xde_lists,list_addr,keyws_reg,coortype):
		...
		if len(shp_list) == 2:
			...
			if shp_list[1] == '%4':
				if xde_lists['disp'].count(shp_list[2]) == 1:
					...
					print('error: line {}, disp var \'{}\' should be found in disp declaration line {}.'.format(list_addr['shap'][shap_i],shp_list[2],list_addr['disp']))
				else:
					print('error: line {}, duplicated disp declaration \'{}\'.'.format(list_addr['disp'],shp_list[2]))
	...
	if xde_lists['code']['BFmate'] != None:
		release_code(xde_lists,list_addr,'BFmate',file,pfelacpath)
	...
	if xde_lists['shap'] != None:
		release_shap(xde_lists,list_addr,file,pfelacpath,shap_tag,dim)
	else:
...
def release_code(xde_lists,list_addr,code_place,file,pfelacpath):
	i = 0
	for strs in xde_lists['code'][code_place]:
		i += 1
		operator_list = []
		operator_strs = ''
		line_num = list_addr['code'][code_place][i-1]
...
def release_shap(xde_lists,list_addr,file,pfelacpath,shap_tag,dim):
	shap_string1=''
	shap_string2=''
	shap_list = []
	lib_coor_list = ['x','y','z']
	...
	for shaplist in xde_lists['shap']:
		if len(shaplist) == 2:
			...
			print('error: line {}, not enough shap declaration'.format(list_addr['shap'][shap_i]))
