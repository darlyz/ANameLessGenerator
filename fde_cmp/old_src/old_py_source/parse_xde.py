...
def parse_xde(gesname, coortype, keywd_tag, xde_lists, list_addr, keyws_reg,file):
	...
	if xde_lists['shap'] != None:
		...
		for shp_list in shap_list:
			if len(shp_list) == 2:
				if (shp_list[0][0] == '%' and shp_list[0][1] != '1') or \
				   (shp_list[1][0] == '%' and shp_list[1][1] != '2'):
		...
		for code_strs in xde_lists['code'][code_place]:
			...
			if regxrp != None:
				...
				# 3.5.2 parsing assignment
					...
					if  'vect' in xde_lists and opr_list[1] in xde_lists['vect']:
						temp_str += '_i='+opr_list[2]+'['
