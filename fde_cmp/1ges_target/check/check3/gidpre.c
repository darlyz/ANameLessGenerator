...
int gidpre(void)
{
    ...
    */
    if ((fp = fopen("./check3.gid/check3.dat","r"))==NULL)
    {
        printf("cat:cannot open %s\n","./check3.gid/check3.dat");
        /*system("pause");*/ exit(1);
    }
    ...
    unoda = (double *) calloc(mkd*4,sizeof(double));
    for (m=0; m<(mkd*4); m++) unoda[m] = 0.0;
    m = getid(fp,dofa,knode,ida);
    m = getrd(fp,dofa,knode,ubfa);
    mkd = knode*dofa*0;
    m = getrd(fp,dofa,knode,&unoda[mkd]);
    mkd = knode*dofa;
    for (m=0; m<mkd; m++) unoda[mkd*inita+m] = unoda[m];
    ntype = 1;
    m = getelem(fp,ntype,em,&elema);
    fclose(fp);
    if ((fp = fopen("check3.mat","r"))==NULL)
    {
        printf("cat:cannot open %s\n","check3.mat");
        /*system("pause");*/ exit(1);
    }
