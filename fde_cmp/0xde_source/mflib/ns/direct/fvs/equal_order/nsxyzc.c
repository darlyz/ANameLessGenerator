...
int num;
{
    ...
    double x[9],y[9],z[9];
    double xc[4],uc[4];
    double fx13[4],fx16[4],fx18[4],fx27[4],fx38[4],fx57[4];
    double fu13[4],fu16[4],fu18[4],fu27[4],fu38[4],fu57[4];
    double x12[4],u12[4],flux12,flux21,flux12r,r12,r21;
    double x14[4],u14[4],flux14,flux41,flux14r,r14,r41;
    double x15[4],u15[4],flux15,flux51,flux15r,r15,r51;
    double x23[4],u23[4],flux23,flux32,flux23r,r23,r32;
    double x26[4],u26[4],flux26,flux62,flux26r,r26,r62;
    double x34[4],u34[4],flux34,flux43,flux34r,r34,r43;
    double x37[4],u37[4],flux37,flux73,flux37r,r37,r73;
    double x48[4],u48[4],flux48,flux84,flux48r,r48,r84;
    double x56[4],u56[4],flux56,flux65,flux56r,r56,r65;
    double x58[4],u58[4],flux58,flux85,flux58r,r58,r85;
    double x67[4],u67[4],flux67,flux76,flux67r,r67,r76;
    double x78[4],u78[4],flux78,flux87,flux78r,r78,r87;
    int mode;
    mode = 1;
    ncoor = 3;
    nnode = 8;
    ...
    eload[29]=0.0;
    eqn[(2-1)*32+2-1]=(r12-1.)*flux12+(r14-1.)*flux14+(r15-1.)
                      *flux15;
    eqn[(2-1)*32+6-1]=(1.-r12)*flux12;
    eqn[(2-1)*32+14-1]=(1.-r14)*flux14;
    eqn[(2-1)*32+18-1]=(1.-r15)*flux15;
    eload[2]=0.0;
    eqn[(6-1)*32+6-1]=(r23-1.)*flux23+(r21-1.)*flux21+(r26-1.)
                      *flux26;
    eqn[(6-1)*32+10-1]=(1.-r23)*flux23;
    eqn[(6-1)*32+2-1]=(1.-r21)*flux21;
    eqn[(6-1)*32+22-1]=(1.-r26)*flux26;
    eload[6]=0.0;
    eqn[(10-1)*32+10-1]=(r34-1.)*flux34+(r32-1.)*flux32+(r37-1.)
                        *flux37;
    eqn[(10-1)*32+14-1]=(1.-r34)*flux34;
    eqn[(10-1)*32+6-1]=(1.-r32)*flux32;
    eqn[(10-1)*32+26-1]=(1.-r37)*flux37;
    eload[10]=0.0;
    eqn[(14-1)*32+14-1]=(r41-1.)*flux41+(r43-1.)*flux43+(r48-1.)
                        *flux48;
    eqn[(14-1)*32+2-1]=(1.-r41)*flux41;
    eqn[(14-1)*32+10-1]=(1.-r43)*flux43;
    eqn[(14-1)*32+30-1]=(1.-r48)*flux48;
    eload[14]=0.0;
    eqn[(18-1)*32+18-1]=(r58-1.)*flux58+(r56-1.)*flux56+(r51-1.)
                        *flux51;
    eqn[(18-1)*32+30-1]=(1.-r58)*flux58;
    eqn[(18-1)*32+22-1]=(1.-r56)*flux56;
    eqn[(18-1)*32+2-1]=(1.-r51)*flux51;
    eload[18]=0.0;
    eqn[(22-1)*32+22-1]=(r65-1.)*flux65+(r67-1.)*flux67+(r62-1.)
                        *flux62;
    eqn[(22-1)*32+18-1]=(1.-r65)*flux65;
    eqn[(22-1)*32+26-1]=(1.-r67)*flux67;
    eqn[(22-1)*32+6-1]=(1.-r62)*flux62;
    eload[22]=0.0;
    eqn[(26-1)*32+26-1]=(r76-1.)*flux76+(r78-1.)*flux78+(r73-1.)
                        *flux73;
    eqn[(26-1)*32+22-1]=(1.-r76)*flux76;
    eqn[(26-1)*32+30-1]=(1.-r78)*flux78;
    eqn[(26-1)*32+10-1]=(1.-r73)*flux73;
    eload[26]=0.0;
    eqn[(30-1)*32+30-1]=(r87-1.)*flux87+(r85-1.)*flux85+(r84-1.)
                        *flux84;
    eqn[(30-1)*32+26-1]=(1.-r87)*flux87;
    eqn[(30-1)*32+18-1]=(1.-r85)*flux85;
    eqn[(30-1)*32+14-1]=(1.-r84)*flux84;
    eload[30]=0.0;
    eqn[(3-1)*32+3-1]=(r12-1.)*flux12+(r14-1.)*flux14+(r15-1.)
                      *flux15;
    eqn[(3-1)*32+7-1]=(1.-r12)*flux12;
    eqn[(3-1)*32+15-1]=(1.-r14)*flux14;
    eqn[(3-1)*32+19-1]=(1.-r15)*flux15;
    eload[3]=0.0;
    eqn[(7-1)*32+7-1]=(r23-1.)*flux23+(r21-1.)*flux21+(r26-1.)
                      *flux26;
    eqn[(7-1)*32+11-1]=(1.-r23)*flux23;
    eqn[(7-1)*32+3-1]=(1.-r21)*flux21;
    eqn[(7-1)*32+23-1]=(1.-r26)*flux26;
    eload[7]=0.0;
    eqn[(11-1)*32+11-1]=(r34-1.)*flux34+(r32-1.)*flux32+(r37-1.)
                        *flux37;
    eqn[(11-1)*32+15-1]=(1.-r34)*flux34;
    eqn[(11-1)*32+7-1]=(1.-r32)*flux32;
    eqn[(11-1)*32+27-1]=(1.-r37)*flux37;
    eload[11]=0.0;
    eqn[(15-1)*32+15-1]=(r41-1.)*flux41+(r43-1.)*flux43+(r48-1.)
                        *flux48;
    eqn[(15-1)*32+3-1]=(1.-r41)*flux41;
    eqn[(15-1)*32+11-1]=(1.-r43)*flux43;
    eqn[(15-1)*32+31-1]=(1.-r48)*flux48;
    eload[15]=0.0;
    eqn[(19-1)*32+19-1]=(r58-1.)*flux58+(r56-1.)*flux56+(r51-1.)
                        *flux51;
    eqn[(19-1)*32+31-1]=(1.-r58)*flux58;
    eqn[(19-1)*32+23-1]=(1.-r56)*flux56;
    eqn[(19-1)*32+3-1]=(1.-r51)*flux51;
    eload[19]=0.0;
    eqn[(23-1)*32+23-1]=(r65-1.)*flux65+(r67-1.)*flux67+(r62-1.)
                        *flux62;
    eqn[(23-1)*32+19-1]=(1.-r65)*flux65;
    eqn[(23-1)*32+27-1]=(1.-r67)*flux67;
    eqn[(23-1)*32+7-1]=(1.-r62)*flux62;
    eload[23]=0.0;
    eqn[(27-1)*32+27-1]=(r76-1.)*flux76+(r78-1.)*flux78+(r73-1.)
                        *flux73;
    eqn[(27-1)*32+23-1]=(1.-r76)*flux76;
    eqn[(27-1)*32+31-1]=(1.-r78)*flux78;
    eqn[(27-1)*32+11-1]=(1.-r73)*flux73;
    eload[27]=0.0;
    eqn[(31-1)*32+31-1]=(r87-1.)*flux87+(r85-1.)*flux85+(r84-1.)
                        *flux84;
    eqn[(31-1)*32+27-1]=(1.-r87)*flux87;
    eqn[(31-1)*32+19-1]=(1.-r85)*flux85;
    eqn[(31-1)*32+15-1]=(1.-r84)*flux84;
    eload[31]=0.0;
    for (i=2; i<=ndof; ++i)
        for (j=1; j<=i-1; ++j)
        {
