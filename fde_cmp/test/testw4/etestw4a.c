...
*/
#include "felac.h"
void aew4g2(double *,double *,double *,double *,double *,double *,double *,int,int);
/*  adda函数：集成总体刚度矩阵  */
void adda(int *,double *,int *,int,int *,double *,int,int);
void etestw4a(coor0,dof,nodvar,ubf,elem,matrix,f)
int dof,*nodvar;
double *ubf,*f;
    ...
    */
    ...
    for (ityp=1; ityp<=ntype; ++ityp)
    {
        ...
        for(ne=1; ne<=nelem; ++ne)
        {
            ...
            {
            case 1 :
                aew4g2(r,coef,prmt,es,em,ec,ef,ne,ibegin);
                break;
            }
