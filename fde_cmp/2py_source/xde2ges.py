...
def xde2ges(gesname, coor_type, xde_lists, list_addr, gesfile):
  ...
  if 'shap' in xde_lists:
    ...
    for shap_expr in shap_expr_list:
      temp_list = shap_expr.split('=')
      shap_var  = temp_list[0]
      shap_exp  = temp_list[1]
  ...
  if 'coef_shap' in xde_lists:
    ...
    for shap in xde_lists['coef_shap'].keys():
      ...
      shap_strs = ''
      for shap_expr in coef_expr_list:
        shap_strs += shap_expr + '\n'
  ...
  if 'gaus' in xde_lists:
    ...
    if xde_lists['gaus'][0] == 'g':
      ...
      or ges_shap_type[0].lower()=='q' \
      or ges_shap_type[0].lower()=='c':
  ...
  for code_strs in xde_lists['code'][code_place]:
    ...
    if regxrp == None:
      ...
      # Tensor expr summation
        ...
        left_vara = vect_expr.split('=')[0].strip()
        righ_expr = vect_expr.split('=')[1].strip().strip(';')
      ...
      # complex expr expansion
        ...
        left_vara = cplx_expr.split('=')[0].strip()
        righ_expr = cplx_expr.split('=')[1].strip().strip(';')
      ...
      # the operator resault assignment
        ...
        and oprt_expr.find('vol') == -1 :
          ...
          and left_vara[-1] == ']' :
            ...
            if left_vara.count('_') == 0: # may be fault tackle
              ...
              var = left_vara.lstrip('[').rstrip(']').split('_')[0]
              row = int(xde_lists['fmatr'][var][0])
              clm = int(xde_lists['fmatr'][var][1])
              for ii in range(row):
                for jj in range(clm):
        ...
        left_vara = tnsr_expr.split('=')[0].strip()
        righ_expr = tnsr_expr.split('=')[1].strip()
          ...
          and righ_expr[-1] == ']':
            ...
            if left_vara.count('_') == 0:
              ...
              and righ_vara in xde_lists['fmatr']:
                ...
                row = int(xde_lists['fmatr'][righ_vara][0])
                clm = int(xde_lists['fmatr'][righ_vara][1])
        ...
        and  left_vara[-1] == ']' :
          ...
          and righ_expr[-1] == ']':
            ...
            if left_vara.count('_') == 1:
              ...
              left_vara = left_vara.split('_')[0]
              row = int(xde_lists['fmatr'][left_vara][0])
              clm = int(xde_lists['fmatr'][left_vara][1])
              for ii in range(row):
                for jj in range(clm):
